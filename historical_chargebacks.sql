...
SELECT 
    ...
    LEFT( RTRIM( DATENAME( MONTH, TBD.TimeByDay ) ), 3 ) + ', ' + LTRIM(STR(YEAR(TBD.TimeByDay))) AS [Month],
    ...
    END AS [Cap Project],
    CASE
        WHEN P.[Cap Asset Number(s)] LIKE '%SaaS%' THEN @Y
        ELSE @N
    END AS [SaaS Cap],
    CASE P.[Cap Project]
        WHEN 1 THEN
    ...
    CAST(
        ...
        -- START P.[Cap Task]
            ...
            END
                -- END CASE P.[Cap Task]
            WHEN @N THEN
                CAST( RCR.Rate * TA.[Hours] AS MONEY )
                ELSE 0
        END AS MONEY
    ) AS [Exp Cost],
    --Cap Hours
    ...
    CASE
        ...
        WHEN @Y THEN
            CASE
                WHEN P.[Cap Asset Number(s)] LIKE '%SaaS%' THEN
                    CASE
                        WHEN RCR.CC_CODE NOT LIKE 'C%'
                            AND RCR.CC_CODE <> @ITBOCC THEN @CapSaaSObjCode
                        ELSE @ContractorCapSaaSObjCode
                    END
                ELSE
                    CASE
                        WHEN RCR.CC_CODE NOT LIKE 'C%'
                            AND RCR.CC_CODE <> @ITBOCC THEN @CapObjCode
        ...
        ELSE
            CASE
                ...
                WHEN @PROJECT THEN
                    CASE
                        WHEN RCR.CC_CODE NOT LIKE 'C%'
                            AND RCR.CC_CODE <> @ITBOCC THEN @ProjObjCode
                        ELSE @ContractorProjObjCode
                    END
                ELSE
                    CASE
                        WHEN RCR.CC_CODE NOT LIKE 'C%'
                            AND RCR.CC_CODE <> @ITBOCC THEN @SuptObjCode
        ...
        WHEN @Y THEN
            CASE
                WHEN P.[Cap Asset Number(s)] LIKE '%SaaS%' THEN
                    CASE
                        WHEN RCR.CC_CODE NOT LIKE 'C%'
                            AND RCR.CC_CODE <> @ITBOCC THEN @CapSaaSObjCodeDesc
                        ELSE @ContractorCapSaaSObjCodeDesc
                    END
                ELSE
                    CASE
                        WHEN RCR.CC_CODE NOT LIKE 'C%'
                            AND RCR.CC_CODE <> @ITBOCC THEN @CapObjCodeDesc
        ...
        ELSE
            CASE
                ...
                WHEN @PROJECT THEN
                    CASE
                        WHEN RCR.CC_CODE NOT LIKE 'C%'
                            AND RCR.CC_CODE <> @ITBOCC THEN @ProjObjCodeDesc
                        ELSE @ContractorProjObjCodeDesc
                    END
                ELSE
                    CASE
                        WHEN RCR.CC_CODE NOT LIKE 'C%'
                            AND RCR.CC_CODE <> @ITBOCC THEN @SuptObjCodeDesc
